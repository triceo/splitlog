...
public abstract class FileBasedTestCase extends TestCase {
    ...
    protected void assertEqualContent(final byte[] b0, final File file) throws IOException {
        ...
        try {
            ...
            TestCase.assertEquals("Different number of bytes: ", b0.length, count);
    ...
    protected void assertEqualContent(final char[] c0, final File file) throws IOException {
        ...
        try {
            ...
            TestCase.assertEquals("Different number of chars: ", c0.length, count);
            for (int i = 0; i < count; i++) {
    ...
    protected void checkFile( final File file, final File referenceFile )
            throws Exception {
        TestCase.assertTrue( "Check existence of output file", file.exists() );
    ...
    protected void deleteFile( final File file )
            throws Exception {
        if (file.exists()) {
            TestCase.assertTrue("Couldn't delete file: " + file, file.delete());
        }
    }
    ...
    protected File newFile(final String filename) throws IOException {
        final File destination = new File( FileBasedTestCase.getTestDirectory(), filename );
        /*
